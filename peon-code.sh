...
3. Look before sending: capture the target pane first. If it shows a dialog or a menu (numbered choices, a yes/no question), wait and retry later; never type into it. On the input line, a plain capture makes the greyed hint look identical to typed text, so check with colors kept: tmux capture-pane -pet <id>. Hint text is wrapped in dim or gray escape codes and means the box is empty, so send as normal. Only text with no dim or gray styling is real typed text: hold both the message and the Enter and retry later, because a pasted message appends to the box rather than replacing it.
