...
3. Look before sending: capture the target pane first. If it shows a dialog or a menu (numbered choices, a yes/no question), wait and retry later; never type into it. On the input line, tell the greyed hint apart from text the user actually typed: a hint is drawn only while the box is empty, so send as normal. If real typed text sits in the box, hold both the message and the Enter and retry later, because a pasted message appends to the box rather than replacing it.
